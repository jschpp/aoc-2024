...
use nom::{bytes::complete::tag, character::complete::{self, line_ending}, multi::{many1, separated_list1}, sequence::{separated_pair, terminated}, IResult};
...
fn parse_input(input: &str) -> IResult<&str, (SpecialNumberOrdering, Vec<Vec<u64>>)> {
    ...
    let (input, _) = line_ending(input)?;
    let (input, orders) = separated_list1(line_ending,separated_list1(tag(","), complete::u64))(input)?;
...
fn main() {
    ...
    let middle_sum: u64 = orders.iter().flat_map(|a| {
        let mut b = a.clone();
